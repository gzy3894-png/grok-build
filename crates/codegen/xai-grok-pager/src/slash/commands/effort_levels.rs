...
///
/// - `mark_active` + `current_effort` mark the current session effort with `（当前）`.
...
pub(crate) fn build_effort_arg_items(
    ...
    options
        ...
        .map(|(idx, option)| {
            ...
            let active_suffix = if active { "（当前）" } else { "" };
